 ...
 * -------------------------------------<br>
 * 
 * @author IvanMalison 						<br>
 * 		   San Francisco					<br>
 * 		   IvanMalison@gmail.com			<br>
 * 		   https://github.com/IvanMalison	<br>
 * @code https://github.com/IvanMalison/Pool/blob/master/src/misc/Polygon2D.java<br>
 */
...
public class Polygon2D implements Shape {
	...
	public Polygon2D scale(double scale) {
		...
		for (int i = 0; i < npoints; i++) {
			polygon.xpoints[i] *= scale;
			polygon.ypoints[i] *= scale;
		}
		
		return polygon;
	}
	
	public List<Vector2> nearestEdges(Vector2 point, int n) {
		List<Vector2> edges = getAllEdges();
		List<Vector2> nearestEdges = new ArrayList<>();
		
		for (int i = 0; i < n; i++) {

			Vector2 nearest = edges.get(0);
			double min = nearest.distance(point);
			
			for (int j = 1; j < edges.size(); j++) {
				
				Vector2 edge = edges.get(j);
				double distance = edge.distance(point);
				
				if (distance > min) {
					nearest = edge;
					min = edge.distance(point);
				}
				
			}
			
			nearestEdges.add(nearest);
	...
	public Vector2 getNextPointOnBounds(Vector2 p) {
		Line line = getNearestSideToPoint(p);
		return line.getNormalContactPoint(p);
	}
	
	public List<Line> getAllSides() {
		List<Vector2> edges = getAllEdges();
		...
		for (int i = 0; i < edges.size(); i++) {
			...
			Line line = new Line(edges.get(i), edges.get(next));
			lines.add(line);
	...
	public Line getNearestSideToPoint(Vector2 p) {
		List<Vector2> edges = getAllEdges();
		List<Line> sides = new ArrayList<>();
		Map<Double,Line> distmap = new HashMap<>();
		...
		for (int i = 0; i < edges.size(); i++) {
			...
			//calcualte distance between point and line
			double distance = line.getDistanceBetweenPointAndLine(p);
			distmap.put(distance, line);
			
		}

		//search for the minimum disctance and return line
		Iterator<Double> it = distmap.keySet().iterator();
		double min = it.next();
		
		while (it.hasNext()) {
			double dist = it.next();
			Line line = distmap.get(dist);
	...
	public List<Vector2> getAllEdges() {
		List<Vector2> edges = new ArrayList<>();
		
		for (int i = 0; i < npoints; i++) {
			edges.add(new Vector2(xpoints[i], ypoints[i]));
		}
	 ...
	 * <code>Shape</code>. Only SEG_MOVETO, SEG_LINETO, and SEG_CLOSE point types
	 * are returned by the iterator. Since polygons are already flat, the
	 * <code>flatness</code> parameter is ignored. An optional
	 * <code>AffineTransform</code> can be specified in which case the coordinates
	...
	class Polygon2DPathIterator implements PathIterator {
		...
		/**
		 * Moves the iterator forwards, along the primary direction of traversal, to the
		 * next segment of the path when there are more points in that direction.
		 */
		public void next() {
			index++;
		}
		 ...
		 * iteration. The return value is the path segment type: SEG_MOVETO, SEG_LINETO,
		 * or SEG_CLOSE. A <code>float</code> array of length 2 must be passed in and
		 * can be used to store the coordinates of the point(s). Each point is stored as
		 * a pair of <code>float</code> x,&nbsp;y coordinates. SEG_MOVETO and SEG_LINETO
		 * types return one point, and SEG_CLOSE does not return any points.
		 * 
		 * @param coords a <code>float</code> array that specifies the coordinates of
		 *               the point(s)
		 * @return an integer representing the type and coordinates of the current path
		 *         segment.
		 ...
		 * Returns the coordinates and type of the current path segment in the
		 * iteration. The return value is the path segment type: SEG_MOVETO, SEG_LINETO,
		 * or SEG_CLOSE. A <code>double</code> array of length 2 must be passed in and
		 * can be used to store the coordinates of the point(s). Each point is stored as
		 * a pair of <code>double</code> x,&nbsp;y coordinates. SEG_MOVETO and
		 * SEG_LINETO types return one point, and SEG_CLOSE does not return any points.
		 * 
		 * @param coords a <code>double</code> array that specifies the coordinates of
		 *               the point(s)
		 * @return an integer representing the type and coordinates of the current path
		 *         segment.
		 * @see PathIterator#SEG_MOVETO
